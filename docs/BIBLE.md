...
1 to 6 Learning Shrines (buildings the player enters to study the region's concept)
1 to 6 Challenge Gates (obstacles on roads that block progression until solved)
...
Learning Shrines
Six dedicated shrines, one per lesson topic:
...
Challenge Gates
Six gates, one after each shrine. Each gate tests exactly the concept from the shrine immediately before it. The player cannot skip a shrine and pass the gate after it. Knowledge is verified in order.
  Gate 1 (after Shrine 1): Variable assignment and string challenge
  Gate 2 (after Shrine 2): Strings, integers, and boolean assignment challenge
  Gate 3 (after Shrine 3): Data type identification challenge (all four types)
  Gate 4 (after Shrine 4): Variable syntax bug fix challenge
  Gate 5 (after Shrine 5): Type conversion challenge
  Gate 6 (after Shrine 6): f-string formatting challenge. This is the final gate and it blocks both the Boss Bug and the Boss Chamber. The player must pass it before the boss encounter becomes accessible.
...
Roaming Bugs
Found wandering the open map between shrines and gates as living Phaser graphics entities (not map tiles)
There is always exactly one roaming bug active in a region at all times
When a roaming bug is defeated it fades out and respawns at a new random passable tile after 60 seconds
Triggering a battle: the bug does not require the player to walk into it. If the player moves within 3 tiles (Manhattan distance) of the bug, it automatically triggers the challenge panel. The player cannot step past it without engaging.
Challenge difficulty scales with the player's progress. The more shrines completed, the harder the roaming bug snippet becomes. This means a player who skips shrines faces the easiest variant, while a player who has studied everything faces a harder one. The roaming bug is the only challenge in Region 1 that scales this way.
Roaming bug difficulty tiers (Region 1):
  Base (no shrines completed): single syntax error, one line affected
  Tier 1 (Shrine 1 complete): two-variable assignment with a print, tests variable and string knowledge
  Tier 2 (Shrine 1 + Shrine 2): string concatenation with type mismatch, tests string and integer knowledge
  Tier 3 (Shrine 1 + Shrine 2 + Shrine 3): multi-variable snippet with type error, tests all numeric and string types
...
Boss Bugs are always the hardest variant. There is no difficulty scaling for boss bugs. The snippet always contains multiple errors spanning all Region 1 concepts. If a player reaches the Boss Bug without studying the shrines, the challenge will reflect that difficulty gap. This is intentional design: the boss is a final exam, not a beginner exercise.
...
Before building any challenge system, every Region 1 challenge must be fully defined. These 9 challenges represent the complete Region 1 content and serve as the blueprint for the challenge engine in V1.

Gate-to-Challenge Alignment Table
Each gate in Region 1 has exactly one challenge assigned to it. The challenge tests the concept from the shrine immediately before that gate on the linear path.

  Gate 1 (col 7,  row 5)  -> ch1: Power Up Terminal A       -> Shrine 1: Variables
  Gate 2 (col 12, row 5)  -> ch2: Power Up Terminal B       -> Shrine 2: Strings
  Gate 3 (col 17, row 5)  -> ch3: Identify the Data Types   -> Shrine 3: Integers and Floats
  Gate 4 (col 18, row 11) -> ch4: Fix the Broken Variable   -> Shrine 4: Booleans
  Gate 5 (col 13, row 11) -> ch9: Convert the Values        -> Shrine 5: Type Conversion
  Gate 6 (col 8,  row 11) -> ch5: String Formatting         -> Shrine 6: f-strings (blocks Boss)

  Roaming Bug             -> ch6 (scaled variant)           -> scales with shrine progress
  Boss Bug (col 6, row 11)-> ch7: Boss Bug                  -> all Region 1 concepts (always hardest)
  Boss Chamber (cols 2-3) -> ch8: Who Are You               -> all Region 1 concepts combined
...
Challenge 9: Convert the Values
Type: Fill-in-the-blank with type conversion functions
Gate: Challenge Gate 5 (after Shrine 5: Type Conversion)
Concept: Type conversion using int(), str(), bool()
Lesson Reference: Code Library: Type Conversion
...
Expected output: 42 98.6 True
ARIA intro: Three values. Wrong types. Convert them. You have the tools now.
ARIA success: All three converted. Type functions work on any value. Gate open.
Validation: score must be int and equal 42; label must be str and equal "98.6"; flag must be bool and True.

DESIGN NOTE: Challenge 9 is the only gate challenge in Region 1 with a sentinel validation (type checks rather than pure stdout match). The player cannot pass by guessing output strings; they must actually call the right conversion function. This is intentional: type conversion is a precision concept that cannot be approximated.
...
All 9 Region 1 challenges available as standalone Practice exercises
...
Region 1: The Origin Node, fully playable with 6 shrines, 6 gates, roaming bug, boss bug, and boss challenge
...
Shrines: 6 (one per topic: Variables, Strings, Integers/Floats, Booleans, Type Conversion, f-strings)
Shrine Topics: 6
Challenges: 9 total: 6 gate challenges, 1 roaming bug (4 difficulty variants), 1 boss bug, 1 boss challenge
...
Shrine 1: Shrine of Variables

This shrine sits at the start of Region 1's top road, just after spawn. It teaches variable assignment. The player must visit before Gate 1 becomes approachable. ARIA greets the player at the door: this is the only place in the region where the lights still work.
...
Shrine: Shrine of Variables
Order: 1 of 1
Pairs With: Challenge Gate 1 (Challenge 1)
...
Topic 2: Data Types
Shrine: Shrine of Variables
Order: 1 of 1 (covered as part of the Variables shrine; players see all four types before Gate 1)
Pairs With: Challenge Gate 1 (Challenge 1)
...
Shrine: Shrine of Conversion (Shrine 5)
Order: 1 of 1
Pairs With: Challenge Gate 5 (Challenge 9)
...
Shrine 2: Shrine of Strings

This shrine sits on the top road after Gate 1. It teaches strings in depth. Players unlock it after solving Gate 1.

Shrine 3: Shrine of Numbers

This shrine teaches integers and floats. Located on the top road after Gate 2.

Shrine 4: Shrine of Booleans

This shrine teaches booleans and their role in programming decisions. Located at the far right of the top road, after Gate 3.

Shrine 5: Shrine of Conversion

This shrine teaches type conversion. Located on the bottom road (accessible via the right connector after passing Gate 3), after Gate 4.

Shrine 6: Shrine of Expression

This shrine sits on the bottom road near the end of the path, after Gate 5. It teaches f-strings and string formatting. Players must solve Gate 5 before reaching it. Gate 6 is just after this shrine and blocks the Boss Bug and Boss Chamber. ARIA comments that this is where the terminals start talking back, not just listening.
...
Shrine: Shrine of Expression (Shrine 6)
Order: 1 of 1
Pairs With: Challenge Gate 6 (Challenge 5)
...
Topic 5: Lists
Shrine: Shrine of Expression (Shrine 6, preview topic only)
Order: 2 of 2 within the shrine, but preview only
...
Every challenge in Region 1 tests exactly one thing the player has been taught. The difficulty comes from applying knowledge correctly, not from encountering something new. A player who thoroughly reads the shrine before each gate should complete every challenge on the first or second attempt.

Challenge to Lesson Pairing
- Challenge 1: Power Up Terminal A     -> Shrine 1: Variables and Assignment
- Challenge 2: Power Up Terminal B     -> Shrine 2: Strings (and integer/boolean assignment)
- Challenge 3: Identify the Data Types -> Shrine 3: Integers and Floats (all four types)
- Challenge 4: Fix the Broken Variable -> Shrine 4: Booleans (variable syntax and type bugs)
- Challenge 9: Convert the Values      -> Shrine 5: Type Conversion
- Challenge 5: String Formatting       -> Shrine 6: f-strings
- Challenge 6: Syntax Gnat (Roaming)   -> Scales with shrine progress (see Bug Difficulty Tiers)
- Challenge 7: Boss Bug                -> All Region 1 concepts combined (always hardest)
- Challenge 8: Boss Challenge          -> All Region 1 concepts combined (capstone)

Gate Order and Linear Path
The player must complete challenges in this exact sequence. Each gate blocks the path to the next shrine.
  Spawn -> Shrine 1 -> Gate 1 -> Shrine 2 -> Gate 2 -> Shrine 3 -> Gate 3
        -> (right connector) -> Shrine 4 -> Gate 4 -> Shrine 5 -> Gate 5
        -> Shrine 6 -> Gate 6 -> Boss Bug -> Boss Chamber

Roaming Bug Difficulty Tiers
The roaming bug challenge (ch6) scales with how many shrines the player has completed. The variants are selected automatically at the time the player walks within 3 tiles of the bug.
  No shrines completed: base ch6, single syntax error (missing quote)
  Shrine 1 complete:    ch6_v1, two variables and print, tests variable and string knowledge
  Shrine 1+2 complete:  ch6_v2, string and integer combination, type mismatch error
...
- Challenge 2: Fill-in-the-blank. Player assigns integer and boolean with correct types.
...
Standard mode (stdout match): ch1, ch2, ch5, ch6, ch6_v1, ch6_v2, ch6_v3, ch7
Sentinel mode (type-check via validation_code, output must end with __VALID__): ch3, ch4, ch8, ch9

Challenges ch3, ch4, and ch8 use sentinel mode because their correctness depends on variable types, not just printed values. Challenge ch9 uses sentinel mode because calling the wrong conversion function may sometimes produce the right output by coincidence; the validator explicitly checks isinstance() for each variable.

For ch8 (Boss Challenge), exact string values are flexible as long as all four data types are correctly used and the f-string references at least two variables.
...
- Lesson 1: Variables and Assignment      -> Challenge 1 (Gate 1)
- Lesson 2: Strings                       -> Challenge 2 (Gate 2)
- Lesson 3: Integers and Floats           -> Challenge 3 (Gate 3)
- Lesson 4: Booleans                      -> Challenge 4 (Gate 4)
- Lesson 5: Type Conversion               -> Challenge 9 (Gate 5)
- Lesson 6: String Formatting (f-strings) -> Challenge 5 (Gate 6)
- Lesson 7: Lists                         -> Boss Challenge and Region 5 preview
...
Road structure: Snake path (not a loop)
  Top road:         row 5, columns 2 to 22 (left to right, spawn at col 2)
  Right connector:  column 22, rows 5 to 11 (descends from top road to bottom road)
  Bottom road:      row 11, columns 2 to 22 (right to left, ends at boss chamber)

Player progresses: left along top road -> right connector -> right along bottom road -> boss area

Top road (left to right, row 5):
  Player spawn:  col 2
  Shrine 1:      cols 5-6, rows 3-4  (above road)   -> Variables and Assignment
  Gate 1:        col 7,  row 5
  Shrine 2:      cols 9-10, rows 3-4 (above road)   -> Strings
  Gate 2:        col 12, row 5
  Shrine 3:      cols 13-14, rows 3-4 (above road)  -> Integers and Floats
  Gate 3:        col 17, row 5
  Right connector begins at col 22

Bottom road (right to left, row 11):
  Shrine 4:      cols 20-21, rows 12-13 (below road) -> Booleans
  Gate 4:        col 18, row 11
  Shrine 5:      cols 15-16, rows 12-13 (below road) -> Type Conversion
  Gate 5:        col 13, row 11
  Shrine 6:      cols 10-11, rows 12-13 (below road) -> f-strings
  Gate 6:        col 8,  row 11  (final gate, blocks boss area)
  Boss Bug:      col 6,  row 11
  Boss Chamber:  cols 2-3, row 11
...
Layer 12: Six-Shrine Map Expansion and Full Lesson Content
Status: COMPLETE (2026-05-22)

Expanded the Origin Node from 2 shrines to 6, one per lesson topic from BIBLE
Section XIV. Each shrine carries its complete word-for-word lesson content.

Map changes (origin_node.js):
  Four shrines above the top road (rows 3–4):
    Shrine 1 (Variables):        cols  5– 6  ← existing position kept
    Shrine 2 (Strings):          cols  9–10  ← new
    Shrine 3 (Integers/Floats):  cols 13–14  ← new
    Shrine 4 (Booleans):         cols 17–18  ← new
  Two shrines inside the road loop:
    Shrine 5 (Type Conversion):  cols 15–16, rows 7–8  ← new
    Shrine 6 (f-strings):        cols  9–10, rows 8–9  ← existing position kept

  Row 3: added 3 at cols 9,10,13,14,17,18
  Row 4: added 3 at cols 9,10,13,14,17,18
  Row 7: added 3 at cols 15,16
  Row 8: added 3 at cols 15,16
  ORIGIN_NODE_OBJECTS: registered shrine2–shrine6 entries

Shrine overlays:
  OriginNodeScene._renderShrineOverlays() auto-detects all SHRINE tiles from the
  map array. Adding tiles to the map automatically generates the 6th building
  overlay without any scene code changes required.

Shrine data (region1_shrines.js):
  Complete rewrite. 6 shrines, each with one focused topic containing the full
  lesson text from BIBLE Section XIV:
    shrine1 - Variables and Assignment   (Shrine of Variables)
    shrine2 - Strings                    (Shrine of Strings)
    shrine3 - Integers and Floats        (Shrine of Numbers)
    shrine4 - Booleans                   (Shrine of Booleans)
    shrine5 - Type Conversion            (Shrine of Conversion)
    shrine6 - f-strings                  (Shrine of Expression)
  Each shrine includes: explanation sections, worked code examples, common
  mistakes, and a shrine challenge with VALIDATION criteria.

Dialogue (region1_dialogue.js):
  Added 4 new proximity zones:
    approach_shrine2 (Strings, col 9.5, row 3.5, radius 2)
    approach_shrine3 (Integers/Floats, col 13.5, row 3.5, radius 2)
    approach_shrine4 (Booleans, col 17.5, row 3.5, radius 2)
    approach_shrine5 (Type Conversion, col 15.5, row 7.5, radius 2)
  Updated shrine6 zone comment; renamed approach_shrine2 → approach_shrine6 for
  the f-strings shrine at (9–10, 8–9).

No changes required to:
  OriginNodeScene.js     - shrine overlay renderer is data-driven
  main.js                - _identifyShrine() uses cols.includes + rows.includes,
                           disambiguates all 6 shrines without modification
  game.html              - all three files already in load order

---

Layer 13: Six Gates, Roaming Bug System, Boss Bug Hardening
Status: COMPLETE (2026-05-22)

This layer redesigned the Region 1 challenge architecture around a strict shrine-then-gate linear path, added a living roaming bug system with proximity triggering and respawn, scaled roaming bug difficulty to shrine progress, and enforced the boss bug as always-hardest.

Map redesign (origin_node.js):
  Road structure changed from rectangular loop to snake path:
    Top road (row 5, left to right): spawn at col 2, shrines 1-3 above road (rows 3-4),
      Gate 1 at col 7, Gate 2 at col 12, Gate 3 at col 17, right connector at col 22
    Right connector (col 22, rows 5-11): descends to bottom road
    Bottom road (row 11, right to left): shrines 4-6 below road (rows 12-13),
      Gate 4 at col 18, Gate 5 at col 13, Gate 6 at col 8,
      Boss Bug at col 6, Boss Chamber at cols 2-3
  Spawn moved to col 2, row 5 (top road start)
  ORIGIN_NODE_OBJECTS updated: all 6 shrines, 6 gates, bossBug, bossChamber

Shrine repositioning (region1_shrines.js):
  shrine4: cols [20,21], rows [12,13]
  shrine5: cols [15,16], rows [12,13]
  shrine6: cols [10,11], rows [12,13]
  shrine1, shrine2, shrine3: unchanged at rows 3-4

Challenge data (region1_challenges.js):
  Boss bug hardening: removed ch7_v0, ch7_v1 variants. ch7 is always the hardest snippet.
    Boss bug is always AG.CHALLENGES['ch7'], never resolved via CHALLENGE_VARIANTS.
  Gate challenge simplification: removed ch4_v0, ch4_v1, ch4_v2. Gates are fixed, not adaptive.
  New challenge ch9: "Convert the Values" (Gate 5 - Type Conversion)
    Type: fill_blank, category: gate
    Prompt: three variables in wrong types (string, float, int), player supplies int(), str(), bool()
    Expected output: '42 98.6 True'
    Validation: sentinel mode, isinstance() checks for all three variables
  New roaming bug variants ch6_v1, ch6_v2, ch6_v3 (tiers 1-3):
    ch6_v1 (requires shrine1): two-variable assignment and print, tests string knowledge
    ch6_v2 (requires shrine1+shrine2): string+integer combination, type mismatch error
    ch6_v3 (requires shrine1+shrine2+shrine3): multi-variable snippet with type errors
  CHALLENGE_VARIANTS updated: ch6 now has 4 entries (base + 3 tiers). ch4 and ch7 removed.
  GATE_CHALLENGES updated: 6 keys, one per gate:
    '7,5'->ch1, '12,5'->ch2, '17,5'->ch3, '18,11'->ch4, '13,11'->ch9, '8,11'->ch5
  BOSS_BUG_CHALLENGE = 'ch7' unchanged

Roaming bug system (OriginNodeScene.js):
  Bugs are Phaser Graphics objects, not map tiles. Managed in this._roamingBugs array.
  this._roamingBugs, this._nextBugId, this._bugAttacking added to constructor.
  _initRoamingBugs(AG): spawns first bug 3 seconds after scene creation.
  _randomBugSpawnPos(minDist): returns a random FLOOR/ROAD/SPAWN tile at least minDist
    tiles (Manhattan) from the player. Returns null if no valid position found.
  _spawnRoamingBug(col, row, AG): creates a Graphics circle sprite with bob and fade-in tweens,
    draws a faint 3-tile danger ring around the bug. Adds entry to _roamingBugs.
  _despawnRoamingBug(id, AG): fades out and destroys bug graphics, resets _bugAttacking=false,
    schedules _spawnRoamingBug at a new random position after 60000ms.
  _checkRoamingBugProximity(playerCol, playerRow, AG): called on every player:moved event.
    Skips if _bugAttacking or inputLocked. Manhattan distance <= ATTACK_RADIUS (3) triggers
    roaming_bug:attack event with the bug's id.
  _listenToBus additions:
    player:moved -> _checkRoamingBugProximity
    roaming_bug:defeated -> _despawnRoamingBug
    challenge:close -> resets _bugAttacking=false (allows re-trigger if still in range)
  _restoreRegion: ORIGIN changed to col 3, row 11 (boss chamber, bottom road)

Challenge routing (main.js):
  _resolveChallenge(id): looks up CHALLENGE_VARIANTS[id]; walks variant list from index 0
    and advances as long as all required shrines in v.requires_shrines are in AG.completedShrines.
    Returns the AG.CHALLENGES entry for the resolved id. Used for gate interactions and roaming bug.
  roaming_bug:attack handler (section 6b): calls _resolveChallenge('ch6') and opens
    challengePanel with { type: 'roaming_bug', bugId: id } as gatePos.
  challenge:solved routing: if gatePos.type === 'roaming_bug', emits roaming_bug:defeated
    with bugId and speaks dismissal line. Gate branch unchanged.
  Gate interaction: uses _resolveChallenge(nextId) for all gates (future-proof; only ch6 currently
    has variants, so all gate challenges resolve directly to their base id).
  Boss bug: AG.challengePanel.open(AG.CHALLENGES[AG.BOSS_BUG_CHALLENGE], null) — direct lookup,
    no resolution, always hardest.

Challenge panel routing (challenge.js):
  roaming_bug category: emits challenge:solved with this.gatePos (carries {type, bugId})
    so the main.js handler can extract the bugId and emit roaming_bug:defeated correctly.
